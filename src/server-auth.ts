...
export const initializeSessionSecurityFn = createServerFn({ method: "POST" }).handler(async () => {
...
export const checkAdminSessionFn = createServerFn({ method: "GET" }).handler(async () => {
  ...
  if (roleError || !roles) {
    ...
    if (
      user.email === "edapt.me@gmail.com" ||
      user.email === "1millionaisuperstars2026@gmail.com"
    ) {
      return true;
    }
    return false;
